  ...
  return (
    <div className=''>
      ...
      <div className='mb-4 grid sm:grid-cols-2'>
        <div>
          <div className='grid relative sm:static gap-4'>
            <div onClick={toggleLightBox}>
              ...
              <Image src={`/` + activeImage.path} width={'600'} height={'600'} className='md:rounded-lg cursor-not-allowed hover:cursor-pointer'/>
              <div
            ...
            <div className='hidden md:grid md:grid-cols-4 md:gap-4'>
              <div onClick={() => {setActiveImage(product.images[0])}}>
                <Image src={`/` + product.images[0].path} width={'600'} height={'600'} className='rounded-lg hover:opacity-50 active:border-2'/>
              </div>
              <div onClick={() => {setActiveImage(product.images[1])}}>
                <Image src={`/` + product.images[1].path} width={'600'} height={'600'} className='rounded-lg hover:opacity-50 active:border-2'/>
              </div>
              <div onClick={() => {setActiveImage(product.images[2])}}>
                <Image src={`/` + product.images[2].path} width={'600'} height={'600'} className='rounded-lg hover:opacity-50 active:border-2'/>
              </div>
              <div onClick={() => {setActiveImage(product.images[3])}}>
                <Image src={`/` + product.images[3].path} width={'600'} height={'600'} className='rounded-lg hover:opacity-50 active:border-2'/>
              </div>
            </div>
          </div>
        </div>
        <div className='m-6 mb-20'>
          <h2 className='text-3xl mb-4 font-extrabold'>{product.title}</h2>
          <p className='text-xl mb-4'>{product.description}</p>
          <h3 className='text-2xl font-extrabold mb-4'>
            ${product.price / 100}
          </h3>
          <div className='flex justify-around basis-[8rem] flex-shrink-0 bg-lightGrayishBlue items-center p-4 rounded-xl'>
            <button
